...
bool LogReader::reopenFD() {
    ...
    if (TC_File::isFileExist(file_)) {
        ...
        last_read_time_ = TNOW;
        auto ret = ::fseek(fd_, 0, SEEK_END);
        auto fileSize = 0;
        if (0 == ret) {
            fileSize = ::ftell(fd_);
        } else {
            throw std::runtime_error("fseek fail!");
        }
...
const std::vector<std::shared_ptr<RawLog>> &LogReader::read() {
    ...
    //TLOG_DEBUG("log-read-size:" << read_size << ", input_pos_:" << input_pos_ << endl);
    time_t now = TNOW;
    if (read_size == 0) {
        //time_t now = time(nullptr);
        if (last_read_time_ + 60 < now) {
            ::fclose(fd_);
            fd_ = NULL;
        }
        return v_;
