...
if [ "$SLAVE" != "true" ]; then
    ...
    cd ${WORKDIR}
    LOG_INFO "copy web to install path: ${TARS_PATH}";
    ...
    sed -i "s/db.tars.com/$MYSQLIP/g" `grep db.tars.com -rl ${TARS_PATH}/web/config/webConf.js`
    sed -i "s/localip.tars.com/$HOSTIP/g" `grep localip.tars.com -rl ${TARS_PATH}/web/config/webConf.js`
    sed -i "s/3306/$PORT/g" `grep 3306 -rl ${TARS_PATH}/web/config/webConf.js`
    sed -i "s/registry.tars.com/$HOSTIP/g" `grep registry.tars.com -rl ${TARS_PATH}/web/config/tars.conf`

    sed -i "s/enableAuth: false/enableAuth: true/g" ${TARS_PATH}/web/config/authConf.js
    sed -i "s/enableLogin: false/enableLogin: true/g" ${TARS_PATH}/web/config/loginConf.js

    sed -i "s/db.tars.com/$MYSQLIP/g" `grep db.tars.com -rl ${TARS_PATH}/web/demo/config/webConf.js`
    sed -i "s/3306/$PORT/g" `grep 3306 -rl ${TARS_PATH}/web/demo/config/webConf.js`

    sed -i "s/enableLogin: false/enableLogin: true/g" ${TARS_PATH}/web/demo/config/loginConf.js

    LOG_INFO "start web";

    cd ${TARS_PATH}/web; pm2 stop tars-node-web; pm2 delete tars-node-web; npm run prd; 
    cd ${TARS_PATH}/web/demo; pm2 stop tars-user-system;  pm2 delete tars-user-system; npm run prd

    LOG_INFO "INSTALL TARS SUCC: http://$HOSTIP:3000/ to open the tars web."
    LOG_INFO "If in Docker, please check you host ip and port."
    LOG_INFO "You can start tars web manual: cd ${TARS_PATH}/web; npm run prd"
